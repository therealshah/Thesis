...
public class SkewMin{
	...
	private static int leftBoundary; // size of how many elements this hash must be greater than/less than to be considered a boundary
	...
	private static void driverRun() throws IOException, Exception{
		...
		-------------------------------------------------------------------------------------------------*/
			System.out.print(leftBoundary +" " + rightBoundary + " ");
			...
			--------------------------------------------------------------------------------------*/
				...
				for (String fileName: fileList)
				{
					...
					if (first){
						...
						run2min(array,md5Hashes);// here we run 2min, ck how similar the documents are to the one already in the system
					}
...
-------------------------------------------------------------------------------------------------------- */
	private static void storeChunks(byte [] array, ArrayList<Long> md5Hashes){
		int start = 0; // starting point
		//System.out.println("TESTING" + localBoundary);
		int current = leftBoundary;// has to be atlead here to be the local minima
		int end  = current  + rightBoundary;  // this is the end of the boundary
		...
		{ 
			...
			{							
				...
				if (i == end)
				{
					...
					current = (leftBoundary - rightBoundary)+ end + 1; // this is where we start finding the new local minima
					start = documentStart; // we will start comparing from here!, since everything before this is a boundary
					end = current + rightBoundary; // this is the new end of the hash boundary
...
-------------------------------------------------------------------------------------------------------- */
	private static void run2min(byte [] array, ArrayList<Long> md5Hashes){
		int start = 0; // starting point
		int current = leftBoundary;// has to be atlead here to be the local minima
		int end  = current + rightBoundary;  // this is the end of the window
		...
		{ 
			...
			{							
				 ...
				 if (i == end)
				{
					...
					current = (leftBoundary - rightBoundary) + end+ 1; // this is where we start finding the new local minima
					start = documentStart; // we will start comparing from here!, since everything before this is a boundary
					end = current + rightBoundary; // this is the new boundary
